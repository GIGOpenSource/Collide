...
必填字段（NOT NULL）：
- mercId      -> 通过 channel_code 关联到 t_payment_channel.merchant_id
- tradeNo     -> order_no
- type        -> pay_type  
- money       -> amount
...
可选字段（允许 NULL）：
- mode        -> pay_mode（选填：sdk模式时传"sdk"）
- payload     -> payload（选填：过滤赔付渠道时传"1"）

info对象（整个对象可选，所有字段允许 NULL）：
- info.playerId    -> player_id
- info.playerIp    -> player_ip  
- info.deviceId    -> device_id
- info.deviceType  -> device_type
- info.name        -> player_name
- info.tel         -> player_tel
- info.payAct      -> player_pay_act

重要提醒：
1. 虽然大白鲨支付文档说 info 内的 playerId、playerIp、deviceId 是必填的，
   但整个 info 对象本身是可选的，所以数据库设计为允许 NULL。

2. 在业务代码中，如果传递了 info 对象，建议验证关键字段的完整性。

3. 玩家ID、IP、设备ID等信息主要用于风控，如果没有这些信息，
   可能会影响支付平台的风控判断，但不会影响支付流程本身。

4. 在查询包含 player_* 字段的数据时，记得处理 NULL 值：
   SELECT * FROM t_payment_order WHERE player_id IS NOT NULL;
   SELECT * FROM t_payment_order WHERE player_id = 'xxx' OR player_id IS NULL;
*/ 